...
DO_BIN=0; DO_RUNNER=0; DO_DB=0; HOST_ARG=""
for arg in "$@"; do
  case "$arg" in
    --runner) DO_RUNNER=1 ;;
    --db)     DO_DB=1 ;;
    --all)    DO_BIN=1; DO_RUNNER=1 ;;
    -*)       echo "error: unknown flag $arg" >&2; exit 2 ;;
    *)        HOST_ARG="$arg" ;;
  esac
done
# Default: binary only.
if [[ $DO_RUNNER == 0 && $DO_DB == 0 ]]; then DO_BIN=1; fi
...
ssh "$HOST" INSTALL_DIR="$INSTALL_DIR" DO_BIN="$DO_BIN" DO_RUNNER="$DO_RUNNER" \
            DO_DB="$DO_DB" 'bash -s' <<'__REMOTE_ROLLBACK__'
set -euo pipefail
cd "$INSTALL_DIR"
