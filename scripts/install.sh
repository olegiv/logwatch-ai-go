...
echo_info "5. Test the analyzer: $BINARY_NAME"
echo_info "6. Set up cron — copy the runner template and customize:"
echo_info "     cp $INSTALL_DIR/scripts/run-cron.sh.example $INSTALL_DIR/run-cron.sh"
echo_info "     chmod 755 $INSTALL_DIR/run-cron.sh"
echo_info "     # edit run-cron.sh to enable the run_job lines for your sites"
echo_info "   Then add ONE cron entry (root cron):"
echo_info "     7 2 * * * $INSTALL_DIR/run-cron.sh >> $INSTALL_DIR/logs/cron.log 2>&1"
echo_info "   See docs/CRON_SETUP.md for details."
echo_info ""
